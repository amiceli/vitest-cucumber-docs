...
Since `6.4.0`, **vitest-cucumber** also supports generic steps via `Step(...)` in `defineSteps`.
A generic step matches any step keyword (`Given`, `When`, `Then`, `And`, `But`) when the step details or expression pattern are the same.

See [Generic steps](/features/step-generic) for details and an example.
