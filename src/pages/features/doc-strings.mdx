...
<Layout title="DocStrings">
    ...
    describeFeature(feature, (f) => {
        f.Scenario(`DocStrings example`, (s) => {
            s.Given(`I use vitest-cucumber`, (ctx, description: string) => {
                expect(description.split('\n')).toEqual([,
                    'vitest-cucumber is a vitest tool',
                    'It provides to use Gherkin'
                ])
            })
            s.Then(`I use vitest`, (ctx, description: string) => {
                expect(description).toEqual('vitest is awesome')
            })
        })
    })
    ~~~
    ...
    describeFeature(feature, ({f}) => {
        f.ScenarioOutline(`DocStrings outline example`, ({Given, Then}, variables) => {
            Given(`I use vitest-cucumber with <framework>`, (ctx, description: string) => {
                expect([
                    'Vue is a front-end framework',
                    'Stencil is a front-end framework',
                ]).toContain(description)
            })
            Then(`I use vitest <version>`, (ctx, description: string) => {
                expect([
    ...
    describeFeature(feature, (f) => {
        f.Scenario(`DocStrings example`, (s) => {
            s.Given(`I use {string}`, (ctx, tool : string, description: string) => {
                expect(description.split('\n')).toEqual([,
                    'vitest-cucumber is a vitest tool',
                    'It provides to use Gherkin'
                ])
